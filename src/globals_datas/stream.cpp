...
void Stream::add_entry(const std::map<std::string, std::string>& map_entry) {
...
Stream::Stream(const std::string& key, const std::map<std::string, std::string>& map_entry) {
    this->key = key;
    this->entries.push_back(map_entry);
}
...
void StreamList::set_entry(const std::vector<std::string>& args){
...
int StreamList::get_stream_index(const std::string& stream_key) {
...
bool StreamList::exist(const std::string& stream_key) {
    return this->get_stream_index(stream_key) != this->streams.size();
};

Stream StreamList::get_by_index(const size_t& index) {
